const context = {
	entries: [
    	{
			name: 'Computer Monitor Light',
			category: 'Office',
			image: 'https://m.media-amazon.com/images/I/51TH4WwyzhL._AC_SX679_.jpg',
			description: 'Quntis Computer Monitor Light | Auto-Dimming, Adjustable Brightness and Temperature',
			price_label: '$$',
			links: [
				{
					price: '$48.99',
					url: 'https://www.amazon.com/dp/B08HMLKS2N'
				}
			],
			purchase_ready: false,
			fund_raising: false,
			taken: true
		},
    	{
			name: 'Ultrasonic Cleaning Pod',
			category: 'Hygiene',
			image: 'https://m.media-amazon.com/images/I/71ah09iE7nL._SX679_.jpg',
			description: 'Ultrasonic Cleaning Pod | Retainer Cleaner | White',
			price_label: '$',
			links: [
				{
					price: '$37.59',
					url: 'https://www.amazon.com/dp/B0DCBMDQ8T'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Vacuum Cleaner',
			category: 'Home',
			image: 'https://target.scene7.com/is/image/Target/GUEST_82d398bd-c97e-408d-aae4-f7cdbfeeea4a?wid=1200&hei=1200&qlt=80&fmt=pjpeg',
			description: 'Shark Stratos Upright Vacuum | Model: AZ3002',
			price_label: '$$$',
			links: [
				{
					price: '$249.99',
					url: 'https://www.target.com/p/shark-stratos-upright-vacuum-with-duoclean-powerfins-hairpro-self-cleaning-brushroll-odor-neutralizer-technology-navy-az3002/-/A-86794826'
				},
        		{
					price: '$279.95',
					url: 'https://www.amazon.com/dp/B0C87H7QKS'
				}
			],
			purchase_ready: false,
			fund_raising: false,
			taken: true
		},
    	{
			name: 'XBox Game Pass',
			category: 'Games',
			image: 'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fi.pinimg.com%2Foriginals%2Fe0%2Fbe%2F50%2Fe0be5066ace675eb18bb130d95be81b9.png&f=1&nofb=1&ipt=82d642d1ae166980cfd1da1c5326b86106e8420b894f3637edf8afc1f346c323&ipo=images',
			description: 'XBox Game Pass | 3 Months',
			price_label: '$',
			links: [
				{
					price: '$30.00',
					url: 'https://www.microsoft.com/en-us/p/xbox-gift-card-digital-code/cfq7ttc0k63h?icid=CNav_Xbox_Gift_Card&activetab=pivot:overviewtab'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'StoveGuard',
			category: 'Home',
			image: 'https://m.media-amazon.com/images/I/71oHWPpPVWL._AC_SX679_.jpg',
			description: 'StoveGuard | Model: KSGG700ESS1 | Black',
			price_label: '$$',
			links: [
				{
					price: '64.99',
					url: 'https://www.amazon.com/dp/B07NVV2G6R'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Orbital Buffer',
			category: 'Auto',
			image: 'https://www.harborfreight.com/media/catalog/product/cache/9fc4a8332f9638515cd199dd0f9238da/5/6/56722_W3.jpg',
			description: '20V Cordless 10 In. Buffer – Tool Only',
			price_label: '$',
			links: [
				{
					price: '$24.99',
					url: 'https://www.harborfreight.com/20v-cordless-10-in-buffer-tool-only-56722.html'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Polishing Bonnets',
			category: 'Auto',
			image: 'https://www.harborfreight.com/media/catalog/product/cache/c5a7cc0816faf52ed720d0b3840ada0e/5/9/59772_W3.jpg',
			description: '10 in. Terry Microfiber and Fleece Polishing Bonnets, 4-Pack',
			price_label: '$',
			links: [
				{
					price: '$6.99',
					url: 'https://www.harborfreight.com/10-in-terry-microfiber-and-fleece-polishing-bonnets-4-pack-59772.html'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Handhelp Jigsaw',
			category: 'Home',
			image: 'https://www.harborfreight.com/media/catalog/product/cache/9fc4a8332f9638515cd199dd0f9238da/6/3/63630_W3.jpg',
			description: '20V Cordless Variable Speed Jig Saw - Tool Only',
			price_label: '$$',
			links: [
				{
					price: '$29.97',
					url: 'https://www.harborfreight.com/power-tools/power-saws/jig-saws/20v-cordless-variable-speed-jig-saw-tool-only-63630.html'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Watch Band',
			category: 'Fashion',
			image: 'http://www.bartonwatchbands.com/cdn/shop/files/V2CaramelTanSSBuckle_1fdaa494-38fb-4543-aa59-4287bcf8f759.jpg?v=1686849160',
			description: 'Weathered Brown Leather Quick Release watch band | 18mm | Silver Buckle',
			price_label: '$',
			links: [
				{
					price: '$30.99',
					url: 'https://www.bartonwatchbands.com/products/weathered-brown-leather'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Watch Band',
			category: 'Fashion',
			image: 'http://www.bartonwatchbands.com/cdn/shop/products/Black_Brighter_767de9cf-0cbe-4d1b-9e06-660be36c642c.jpg?v=1645414167',
			description: 'Black Alligator Leather Quick Release watch band | 20mm | Silver Buckle',
			price_label: '$',
			links: [
				{
					price: '$35.99',
					url: 'https://www.bartonwatchbands.com/products/black-alligator-grain-leather?variant=19225391202358'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'IR Remote Extender',
			category: 'Tech',
			image: 'https://m.media-amazon.com/images/I/41BLwPZBNmL._AC_.jpg',
			description: '1.5m IR Infrared Emitter Extender Cable Extension',
			price_label: '$',
			links: [
				{
					price: '$6.99',
					url: 'https://www.amazon.com/dp/B08C7X7T5M'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Phone Case',
			category: 'Tech',
			image: 'https://m.media-amazon.com/images/I/716mBXWOjeL._AC_SX679_.jpg',
			description: 'Case for iPhone 7 Plus/8 Plus Case with Slide Camera Cover',
			price_label: '$',
			links: [
				{
					price: '$12.99',
					url: 'https://www.amazon.com/dp/B0D1K1VD4P'
				}
			],
			purchase_ready: false,
			fund_raising: false,
			taken: true
		},
    	{
			name: 'Shoe Organizer',
			category: 'Home',
			image: 'https://m.media-amazon.com/images/I/71bf3j8KgVL._AC_SX679_.jpg',
			description: 'Plastic Shoe Storage Organizer Unit | Grey',
			price_label: '$$',
			links: [
				{
					price: '$38.99',
					url: 'https://www.amazon.com/dp/B085SWT6B8'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
    	{
			name: 'Metal Filing Cabinet',
			category: 'Office',
			image: 'https://m.media-amazon.com/images/I/61FLJdqM3XL.__AC_SX300_SY300_QL70_ML2_.jpg',
			description: 'BIZOEIRON 3 Drawer File Cabinet',
			price_label: '$$$',
			links: [
				{
					price: '119.99',
					url: 'https://www.amazon.com/dp/B0C9HLK921'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
		{
			name: 'Replacement Battery Kit',
			category: 'Tech',
			image: 'https://cdn.shopify.com/s/files/1/0045/4092/4007/files/fEbCRUDVTlITGX3R_f6f0b6c9-8b31-4790-bccf-556a62449eef.jpg?v=1733343984&width=1920',
			description: 'MacBook Pro 15" Retina (Late 2013-Mid 2014) Battery',
			price_label: '$$$',
			links: [
				{
					price: '$199.00',
					url: 'https://www.ifixit.com/products/macbook-pro-15-retina-late-2013-mid-2014-battery'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
		{
			name: 'Replacement Trackpad',
			category: 'Tech',
			image: 'https://cdn.shopify.com/s/files/1/0045/4092/4007/products/lEEDjeuJqVQvEsMe_23acebd2-2be8-465d-87c2-a46bbdbcb3bd.jpg?v=1693330816&width=1920',
			description: 'MacBook Pro 15" Retina (Late 2013-Mid 2014) Trackpad',
			price_label: '$$',
			links: [
				{
					price: '$69.99',
					url: 'https://www.ifixit.com/products/macbook-pro-15-retina-late-2013-mid-2014-trackpad'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		},
		{
			name: 'Replacment Rubber Feet',
			category: 'Tech',
			image: 'https://cdn.shopify.com/s/files/1/0045/4092/4007/products/JFuKwUdyRClLBLys_150508b1-9c7f-4dee-8e3c-0c7051dbe379.jpg?v=1667236224&width=1920',
			description: 'MacBook Pro Retina (A1425, A1502, A1398, and A2289) Plastic Feet',
			price_label: '$',
			links: [
				{
					price: '$4.99',
					url: 'https://www.ifixit.com/products/macbook-pro-retina-a1425-a1502-a1398-and-a2289-plastic-feet?variant=39371638440039'
				}
			],
			purchase_ready: true,
			fund_raising: false,
			taken: false
		}
	]
};
